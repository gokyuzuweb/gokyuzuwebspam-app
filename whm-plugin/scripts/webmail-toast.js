...
(function () {
  "use strict";
  var PANEL = window.GWS_PANEL_URL || "https://panel.gokyuzuhosting.com";
  var STORAGE_KEY = "gws_last_notif_shown";

  function getUserEmail() {
    try {
      if (window.rcmail && window.rcmail.env && window.rcmail.env.username) {
        var u = window.rcmail.env.username;
        return u.indexOf("@") > 0 ? u : null;
      }
    } catch (e) {}
    try {
    ...
    // cPanel default: header'da mailto link'i olabilir
    try {
