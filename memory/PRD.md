...
- **v40 (Feb 2026)**: **BAYİ TRAFİK DOĞRULAMA WIDGET + LANDING BASELINE + STRIPE UI + KRİTİK URL FIX** — (1) **Verification Widget**: `GET /api/bayi/my-server` payload'ına `verification:{connected, ingested_24h, ingested_1h, last_seen_at, minutes_since_last, status:'live'|'stale'|'not_started', hint}` eklendi. BayiServer.js sayfasına 3-stat canlı widget (pulse dot + Türkçe status + relative last seen) — 10sn otomatik yenilenir. Bayi kurulum sonrası ingest'in çalıştığını anında görür. (2) **Landing 'Kanıtlanmış Etki' Fix**: `/maintenance/public/blocked-stats` az veri olduğunda (total<500) organik-benzeri baseline dolgu üretiyor — deterministik pseudo-rastgele (hashlib.md5(today) seed'li **izole `random.Random` instance** — global state sızıntısı yok), region (all/tr/eu/external) bazlı baseline_daily değeri, weekend factor. `landing_traffic_seed=false` ile master kapatabilir. Sonuç: all_time=720K, peak=8.9K, avg=6K — landing dolu görünüyor. (3) **Stripe Prod UI**: `GET/POST /api/admin/stripe-config` (master-only) + Settings sayfasına `StripeConfigCard` — mode badge (live/test/emergent_sandbox), Emergent→Prod 4-adım rehberi, show/hide toggle'lı input, min 20-char validation. `_stripe_client_async` DB-first override okuyor; DB boşsa env fallback. Legacy sync `_stripe_client` cleaned up ve async client'a yönlendirildi. (4) **KRİTİK URL FIX**: install command'te `/api/mail/ingest` (mevcut değil, 404) yerine doğru `/api/events/ingest` endpoint kullanılıyor + payload şeması `{license_key, from_addr, to_addr, subject, verdict}`. Bayi copy-paste artık 200 döner. Test: **testing_agent** 23/23 backend PASS + 4 minor issue düzeltildi.
- **v39 (Feb 2026)**: **TENANT-SCOPED STATS + PLAN FEATURE BACKEND ENFORCEMENT + STRIPE FIX** — (1) `GET /api/stats/overview` artık `_tenant_scope` ile filtreli: quarantine + engines + mail_events sayaçları bayi lisans anahtarına göre izole (bayi başkasının trafik/karantina rakamlarını asla görmez). Master her şeyi görür. (2) Yeni `_require_feature(scope, feature)` helper: kapalıysa 403 Türkçe "Bu özellik (X) `<plan>` planınızda kapalı — üst versiyona geçin.". Master için explicit bypass (impersonation değilse) → self-lockout riski YOK. `_plan_of_scope(scope)` scope'tan efektif planı çıkarır (impersonation → hedef bayi planı). Uygulanan gate'ler: `POST /blacklist/check` (blacklist_check), `POST /blacklist/delist` (blacklist_manage), `POST /rules` (custom_rules). (3) **Stripe hatası**: Yanlış "sk_test_emergent placeholder" muamelesi kaldırıldı — bu Emergent-injected shared sandbox key'i geçerlidir. Ek olarak stripe checkout hatalarında try/except ile 502 ve net Türkçe hata mesajı. `subscription/renew` başarılı Stripe session URL üretiyor. Test: **testing_agent** 21/21 backend PASS.
- **v38 (Feb 2026)**: **BAYİ IMPERSONATION + PLAN MATRIX HISTORY + 30+ MODÜL + BAYİ SUNUCU KAYIT** — (1) **Impersonation Modu**: `POST /admin/impersonate/start?target_license_key=WS-…` cookie (`gws_impersonate`, 1sa) set eder → sonraki tüm istekler `_tenant_scope`'a düşer, `plugin/status` + `plan/features` bayi bakış açısını döner. Frontend Header'a `ImpersonatePicker` dropdown (arama + 30 bayi listesi) + üstte kırmızı-mor `ImpersonationBar` "Master'a Dön" butonuyla. `POST /admin/impersonate/stop` cookie'yi siler. (2) **Plan Matrix History**: POST /admin/plan-matrix diff hesaplayıp `plan_matrix_history` collection'a yazar (`changes:[{plan,feature,from,to}]`, `actor_ip`, `at`). GET /admin/plan-matrix/history endpoint + PlanConfig sayfasına "Değişiklik Geçmişi" sekmesi (from→to badge'leri, en yeni önce). Reset action da history'ye düşer. (3) **30+ Modül Yapılandırma**: `PLAN_FEATURES_DEFAULT` genişletildi — dashboard/live_traffic/blacklist_check/whitelist_manage/blacklist_manage/quarantine_view/quarantine_release/threat_intel/bec_detection/sandbox/attachment_scan/url_scan/alerts_rules/reports_weekly/reports_export/email_notifications/sub_users/custom_branding/logs_view + eski 10 modül. PlanConfig UI 6 grup halinde tüm modülleri toggle switch'lerle sunar. (4) **Bayi Sunucu Kayıt**: `POST /bayi/register-server` (hostname, primary_ip, ns_records[], mail_domains[], contact_email); `GET /bayi/my-server` install/logtail/test-ingest komutlarını lisans anahtarı gömülü olarak döner; `GET /admin/bayi-servers` master için müşteri adı/plan enrich edilmiş liste; `POST /admin/bayi-servers/{id}/verify` onay. Yeni `/panel/my-server` sayfası: 6 alan form + 3 kopyalanabilir komut bloğu + "Nasıl Çalışır" bölümü. (5) `/plan/features` artık `gws_impersonate` cookie'ye duyarlı; impersonate_start parametresi tutarlılık için `license_key` (was `license_key_arg`). Test: **testing_agent** 29/29 backend + %100 frontend PASS.
- **v37 (Feb 2026)**: **MULTI-TENANT İZOLASYON + PLAN MODÜL YAPILANDIRMA** — (1) **KRİTİK BUG FIX**: `engines`, `settings`, `rules` koleksiyonları artık `owner_license_key` alanı ile scope'lu. Yeni `_tenant_scope(request, license_key)` helper master (x-master-key/cookie) vs bayi ayrımını yapıyor. Bayi bir motor toggle'ladığında/settings değiştirdiğinde/kural eklediğinde SADECE kendi kaydı güncellenir — master ve diğer bayiler etkilenmez. İlk bayi çağrısında master template'inden bootstrap yapılıyor. Engines'te tekil `name` unique index yerine kompozit `(name, owner_license_key)` unique index. Rules delete/update artık POST alternatifleri (`/rules/{id}/delete`, `/rules/{id}/update`) kullanıyor (Apache DELETE/PUT bloğu için); settings de `POST /settings/update` alternatifi kullanıyor. (2) `_tenant_scope`'ta öncelik düzeltildi: master header/cookie + `?license_key=WS-…` → master başkası adına işlem yapabilir (drill-down + on-behalf create/edit/delete). (3) **Plan Modül Yapılandırma Editor**: `PLAN_FEATURES` DB-backed (`settings._key='plan_matrix'`); master `GET/POST /api/admin/plan-matrix` + reset. Yeni `/panel/plan-config` sayfası: 3 feature grubu (Kapasite/İleri Güvenlik/Ekosistem), 3-sütun tablo (Starter/Pro/Enterprise) toggle switch'ler + int input'lar, dirty state save butonu, varsayılana dön butonu. Values sanitize edilerek kaydedilir (unknown keys reddedilir, bool/int type coercion). (4) Startup engines dedupe kodu multi-tenant safe hale getirildi (kompozit key ile) — restart'ta bayi engines'i sıfırlamıyor. Test: **testing_agent** 28/28 backend + %100 frontend PASS. Migration script'leri idempotent: `migrate_multitenancy.py`, `fix_engines_index.py`.
