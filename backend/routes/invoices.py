...
async def invoice_pdf(tx_id: str, reseller: dict = Depends(current_reseller)):
    tx = await db.payment_transactions.find_one(
        {"id": tx_id, "license_key": reseller["license_key"], "status": "paid"}, {"_id": 0}
    )
    if not tx:
    ...
    header_data = [[
        ...
        Paragraph(f"<b>FATURA</b><br/>{inv_no}", right),
    ]]
    ...
    story.append(t)
    story.append(Paragraph("WHM / cPanel mail güvenliği · gokyuzuwebspam.com", small))
    ...
    parties = [[
        ...
        Paragraph(
            ...
            f"Lisans: <font name='Courier'>{tx.get('license_key', '')[:24]}…</font>",
            body,
        ),
    ]]
    ...
    meta = [
        ["Fatura No", inv_no],
        ["Tarih", issued_pretty],
        ["Ödeme Ref.", (tx.get("session_id") or "-")[:44]],
        ["Plan", f"{plan_name} ({tx.get('billing_period', '-')})"],
    ]
    ...
    line_items = [
        ...
        [
            f"{plan_name} — GökyüzüWebSpam Lisansı ({tx.get('billing_period', 'monthly')})\n"
            f"Sunucu başına, sınırsız e-posta, IP tabanlı doğrulama",
            "1",
    ...
    totals = [
        ["Ara Toplam", _fmt_money(amount, currency)],
        ["KDV (%0)", _fmt_money(0, currency)],
        ["TOPLAM", _fmt_money(amount, currency)],
    ]
    ...
    story.append(Paragraph(
        f"<font backColor='#dcfce7' color='#16a34a'> ● ÖDENDİ </font>  &nbsp; "
        f"<font color='#64748b' size='8'>Bu fatura elektronik ortamda üretilmiştir; imza gerektirmez.</font>",
        body,
    ))
    story.append(Spacer(1, 12 * mm))
    ...
    story.append(Paragraph(
        "Sorularınız için: <b>destek@gokyuzuwebspam.com</b> · "
        "Kurulum: <b>gokyuzuwebspam.com/panel/install</b>",
        small,
    ))
    story.append(Paragraph(
        "GökyüzüWebSpam — WHM/cPanel için ticari mail güvenliği çözümü. Bu fatura ödemenin resmi kanıtıdır.",
        small,
    ))
