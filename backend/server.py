...
async def seed_if_empty() -> None:
    ...
    await db.settings.insert_one({
        ...
        **VersionManifest(
            ...
            changelog="v44.00.08: heartbeat versiyon fix + /api/plugin/heartbeat alias + SMTP tenant scoping",
            release_date=_iso(),
        ).model_dump()
    })
    ...
    demo_licenses = [
        License(
            ...
            last_heartbeat_version="44.00.04",  # v44.00.08 — bogus seed "1.1.0" kaldırıldı
        ).model_dump(),
        License(
            ...
            last_heartbeat_version="44.00.03",  # v44.00.08 — bir sürüm eski (test için ESKİ badge görünsün)
        ).model_dump(),
    ]
...
async def _startup() -> None:
    ...
    # v44.00.08 — Inactive reseller auto-deactivation (30+ gün heartbeat yoksa)
...
# v44.00.08 — Inactive reseller auto-deactivation
...
async def deactivate_inactive_resellers(request: Request, license_key: Optional[str] = None):
    """v44.00.08 — Master manuel olarak inactive-cleanup tetikler.
...
async def engines_cascade_preview(name: str, request: Request, license_key: Optional[str] = None):
    """v44.00.08 — Master motor toggle'ı yapmadan ÖNCE kaç bayiye yansıyacağını
...
async def engines_toggle(name: str, request: Request, license_key: Optional[str] = None):
    ...
    # v44.00.08 — MASTER CASCADE: Master motor toggle'ı YAPARSA, aynı motor
...
async def _smtp_settings(owner_license_key: Optional[str] = None) -> dict:
    """v44.00.08 — Per-tenant SMTP settings.
...
async def _send_email(to_addr: str, subject: str, body: str, from_addr: str = "gokyuzuwebspam@localhost", owner_license_key: Optional[str] = None) -> tuple[bool, str]:
    ...
    v44.00.08 — `owner_license_key` verilirse o bayinin SMTP ayarları kullanılır;
...
async def get_smtp_settings(request: Request, license_key: Optional[str] = None):
    # v44.00.08 — Per-tenant SMTP: bayi kendi ayarını görür (yoksa master fallback).
    scope = await _tenant_scope(request, license_key)
    owner = scope["owner_license_key"] or ""
    # Bayi kendi dokümanı
    doc = None
    ...
    return {
        ...
        "from_addr": doc.get("from_addr", ""),
        "use_tls":   doc.get("use_tls", "starttls"),
        # v44.00.08 — UI için: bayi master'ınkinden mi devraldı?
...
async def put_smtp_settings(payload: SmtpSettingsIn, request: Request, license_key: Optional[str] = None):
    # v44.00.08 — Kayıt, çağıranın tenant scope'una gider (bayi = kendi, master = global default).
    scope = await _tenant_scope(request, license_key)
    owner = scope["owner_license_key"] or ""
...
async def mail_send_test(payload: MailTestIn, request: Request, license_key: Optional[str] = None):
    ...
    v44.00.08 — Tenant-scoped: bayi kendi SMTP ayarlarıyla test eder."""
...
def _read_panel_version() -> str:
    ...
    """
    _PACKAGE_VERSION = "v44.00.08"  # backend bundle içindeki varsayılan (VERSION dosyası bulunamazsa)
...
async def plugin_signal_log(request: Request, limit: int = 20, license_key: Optional[str] = None):
    ...
    v44.00.08 — SECURITY: master-only. Bu liste bayi lisans key'lerini içerir,
...
async def licenses_list(request: Request, license_key: Optional[str] = None):
    # v44.00.08 — SECURITY: master-only. Bayilere lisans listesini SIZDIRMA.
    m = await _is_master(request, license_key)
    if not m.get("is_master"):
...
class HeartbeatPayload(BaseModel):
    ...
    ip: str
    hostname: Optional[str] = ""
    # v44.00.08 — Default'ı boş yap; eğer plugin göndermediyse UI 'v?' göstersin,
...
# v44.00.08 — Alias endpoint: `gws-simple-push` timer /api/plugin/heartbeat
...
async def license_violations(request: Request, limit: int = 100, license_key: Optional[str] = None):
    # v44.00.08 — SECURITY: master-only. İhlaller license_key + IP bilgisi içerir → sızmasın.
    m = await _is_master(request, license_key)
    if not m.get("is_master"):
        raise HTTPException(status_code=403, detail="Master yetkisi gerekli")
...
class DelistRequest(BaseModel):
    ...
    notes: Optional[str] = ""
    # v44.00.08 — Tenant isolation: hangi lisansa/bayiye ait olduğu (master için MASTER_LICENSE_KEY)
...
async def blacklist_delist(payload: DelistRequestIn, request: Request, license_key: Optional[str] = None):
    ...
    # v44.00.08 — Tenant sahibini belirle (master ise MASTER_LICENSE_KEY, bayi ise kendi key'i)
    ...
    for code in payload.provider_codes:
        ...
        req = DelistRequest(
            ...
            owner_license_key=owner_lk,  # v44.00.08 — Tenant isolation
        ).model_dump()
...
async def blacklist_requests(request: Request, license_key: Optional[str] = None):
    # v44.00.08 — Tenant isolation: bayi sadece KENDİ delisting taleplerini görür
    scope = await _tenant_scope(request, license_key)
    q: dict = {}
    if not scope.get("is_master"):
        owner_lk = scope.get("owner_license_key") or ""
...
async def _authorize_delist(req_id: str, request: Request, license_key: Optional[str]):
    """v44.00.08 — Kayıt sahipliğini doğrula (master her şeye erişebilir)."""
    scope = await _tenant_scope(request, license_key)
...
PLAN_FEATURES_DEFAULT = {
    "starter": {
        ...
        "installation_guide": True,  # v44.00.08 — Kurulum Rehberi (herkes görebilir)
        "install_guide": True,       # v44.00.08 — App.js aynı feature key kullanıyor (alias)
        "module_tour": True,         # v44.00.08 — Modül Turu (herkes görebilir)
        "promo_video": True,         # v44.00.08 — Tanıtım videosu (herkes görebilir)
        # === Liste Yönetimi ===
        ...
        "threat_defense_center": False,  # v44.00.08 — Threat Defense Hub (28 modül)
        "threat_defense": False,     # v44.00.08 — App.js alias (starter'da kapalı — pro/enterprise açık)
        ...
        "bounce_digest_export": False,  # v44.00.08 — CSV/XLSX export
        # === Bildirim & Raporlama ===
        ...
        "push_health_widget": True,  # v44.00.08 — Exim push health (herkes görebilir)
        "reseller_analytics_widget": True,  # v44.00.08 — Kişisel Koruma Panosu (herkes)
        # === Güvenlik & Kilit ===
        "idle_lock": True,           # v44.00.08 — Otomatik kilit + PIN
        "pin_change_request": True,  # v44.00.08 — Bayi PIN değişikliği talebi
        # === Yönetim ===
    ...
    "enterprise": {
        # v44.00.08 — ENTERPRISE'da TÜM özellikler AÇIK olmalı (kullanıcı bildirdi:
...
class VerifyLicenseIn(BaseModel):
    ...
    # v44.00.08 — Plugin version (verify sırasında master hangi versiyonun bağlı olduğunu bilsin)
...
async def plugin_verify_license(payload: VerifyLicenseIn, request: Request = None):
    ...
    if payload.license_key:
        ...
        # v44.00.08 — license_key SAĞLANMIŞ ama DB'de aktif eşleşme YOK →
    ...
    if not lic:
        # v44.00.08 — browser_ip'yi de yakala (server_ip = payload.ip, browser_ip = request headers)
        _browser_ip = ""
        try:
            if request is not None:
                _browser_ip = (request.headers.get("x-forwarded-for") or "").split(",")[0].strip()
        ...
        # v44.00.08 — Canonical collection'a da yaz (frontend'in yeni list endpoint'i buradan okur)
        await db.license_violations.insert_one(dict(v))
        await db.violations.insert_one(v)
        asyncio.create_task(_fire_license_alert(v))
    ...
    await db.licenses.update_one(
        ...
        {"$set": {
            ...
            "last_heartbeat_version": payload.version or "44.00.05",  # v44.00.08 — payload'dan al
        }},
    )
    await db.logs.insert_one(ActivityLog(
        source="license", level="info",
