...
class VersionManifest(BaseModel):
    latest_version: str = "44.00.05"
    download_url: str = "https://panel.gokyuzuhosting.com/api/plugin/download"
    sha256: str = ""
    changelog: str = ""
    release_date: str = Field(default_factory=_iso)
    min_supported: str = "44.00.01"
...
async def seed_if_empty() -> None:
    ...
    await db.settings.insert_one({"_key": "version", "version": "44.00.05", "installed_at": _iso()})
    ...
    demo_licenses = [
        License(
            ...
            last_heartbeat_version="44.00.04",  # v44.00.10 — bogus seed "1.1.0" kaldırıldı
        ).model_dump(),
        License(
            ...
            last_heartbeat_version="44.00.03",  # v44.00.10 — bir sürüm eski (test için ESKİ badge görünsün)
        ).model_dump(),
    ]
    ...
    await db.violations.insert_many([
        LicenseViolation(
            ...
            reason="ip_not_allowed", version="44.00.05",
        ).model_dump(),
        LicenseViolation(
...
async def _startup() -> None:
    ...
    # v44.00.10 — Inactive reseller auto-deactivation (30+ gün heartbeat yoksa)
    asyncio.create_task(_inactive_reseller_deactivation_task())
...
# v44.00.10 — Inactive reseller auto-deactivation
async def _inactive_reseller_deactivation_task():
    """30+ gün heartbeat atmamış aktif bayı lisanslarını `active=False` yap.
    Master ve son 30 gün içinde `created_at` olmuş yeni lisanslar hariç tutulur.
    Günde 1 kez çalışır. Idempotent."""
    await asyncio.sleep(180)  # startup'tan 3 dk sonra
    while True:
        try:
            await _inactive_reseller_deactivation_scan()
        except Exception as ex:
            logging.warning(f"inactive_reseller_deactivation scan error: {ex}")
        await asyncio.sleep(86400)  # 24 saat


async def _inactive_reseller_deactivation_scan() -> dict:
    """Tek geçişte inactive bayıları deaktive et. Rapor döner: {scanned, deactivated}."""
    ...
    q = {
        "active": True,
        "license_key": {"$ne": master_env} if master_env else {"$exists": True},
        # Yeni oluşturulmuş bayı (created_at < 30 gün önce) hariç — henüz kurulum yapmamış olabilir
        "$or": [
            {"created_at": {"$exists": False}},
            {"created_at": {"$lt": recent_iso}},
        ],
        # Heartbeat 30+ gün öncesinde VEYA hiç yok
        "$and": [
            {"$or": [
    ...
    deactivated = 0
    scanned = 0
    async for lic in db.licenses.find(q, {"_id": 0, "license_key": 1, "customer_name": 1, "customer_email": 1, "last_heartbeat_at": 1}):
        scanned += 1
        ...
        await db.licenses.update_one(
            ...
            {"$set": {
                "active": False,
                "deactivated_at": _iso(),
                "deactivated_reason": "auto: 30+ days no heartbeat",
            }},
        )
        deactivated += 1
        # Audit + master alert
        try:
            await db.master_alerts.insert_one({
                "id": str(uuid.uuid4()),
                "type": "reseller_auto_deactivated",
                "severity": "info",
                "message": f"🌙 Otomatik deaktive: {lic.get('customer_name') or lk[:14]}... · 30+ gün heartbeat yok",
                "details": {
                    "license_key": lk,
                    "last_heartbeat_at": lic.get("last_heartbeat_at"),
            ...
            pass
    if deactivated:
        logging.info(f"[inactive_reseller_deactivation] scanned={scanned}, deactivated={deactivated}")
    return {"scanned": scanned, "deactivated": deactivated}


# Master için manuel tetikleme endpoint'i (test / audit)
@api.post("/master/deactivate-inactive-resellers")
async def deactivate_inactive_resellers(request: Request, license_key: Optional[str] = None):
    """v44.00.10 — Master manuel olarak inactive-cleanup tetikler.
    Cron zaten günde 1 kez çalışıyor ama audit için endpoint şart."""
    m = await _is_master(request, license_key)
    if not m.get("is_master"):
        raise HTTPException(403, "Sadece master")
    return await _inactive_reseller_deactivation_scan()
...
async def _daily_usom_fetch_task():
    """v44.00.33 — Günlük USOM delta sync: son çalışmadan bu yana 2024+ yıla ait yeni kayıtları çeker.
    İlk çalışmada full 2024+ (min_year=2024) fetch, sonrasında sadece last_max_date sonrası (delta)."""
    ...
    while True:
        try:
            ...
            if now.hour == 3:
                ...
                if not last or last.get("date") != today:
                    from routes.usom import _fetch_usom_iocs, _ingest_usom_items, USOM_FETCH_PROGRESS_KEY
                    last_max = last.get("last_max_date") if last else None
                    try:
                        items, source_url = await _fetch_usom_iocs(
                            min_year=2024,
                            progress_key=USOM_FETCH_PROGRESS_KEY,
                            min_date=last_max,  # delta: only newer than last run
                        )
                        result = await _ingest_usom_items(items, source_url)
                        # En son gelen kaydın tarihini sakla (delta için)
                        new_max = last_max or ""
                        for it in items:
                        ...
                        await db.settings.update_one(
                            ...
                            {"$set": {
                                "date": today,
                                "last_max_date": new_max or last_max,
                                "trigger": "daily_delta_cron",
                                "count": len(items),
                                **result,
                            }},
                            upsert=True,
                        ...
                        await db.settings.update_one(
                            ...
                            {"$set": {"state": "done", "fetched": len(items),
                                      "finished_at": _iso(), "updated_at": _iso(),
                                      "result": result, "source": source_url,
                                      "trigger": "daily_delta_cron"}},
                            upsert=True,
                        )
                        log.info("usom daily delta cron: %d new IOC · last_max=%s · %s",
                                 len(items), new_max, source_url)
                    except Exception as ex2:
                        log.warning("usom daily delta cron failed: %s", ex2)
        except Exception as ex:
            log.warning("usom cron error: %s", ex)
        await asyncio.sleep(3600)


# v44.00.33 — Karantina Retention Auto-Delete (günlük)
async def _daily_quarantine_retention_task():
    """Her gün 02:00 UTC'de karantina ayarlarındaki retention_days'ten eski
    kayıtları siler. auto_delete_enabled=false veya retention_days=0 iken atlanır."""
    await asyncio.sleep(480)  # startup +8dk
    while True:
        try:
            now = datetime.now(timezone.utc)
            if now.hour == 2:
                today = now.date().isoformat()
                last = await db.settings.find_one({"_key": "quarantine_retention_last_run"}, {"_id": 0})
                if last and last.get("date") == today:
                    await asyncio.sleep(3600); continue
                cfg = await db.settings.find_one({"_key": _QUARANTINE_SETTINGS_KEY}, {"_id": 0}) or {}
                enabled = cfg.get("auto_delete_enabled", True)
                rd = int(cfg.get("retention_days") or 30)
                deleted = 0
                if enabled and rd > 0:
                    cutoff = (now - timedelta(days=rd)).isoformat()
                    r = await db.quarantine.delete_many({"received_at": {"$lt": cutoff}})
                    deleted = r.deleted_count
                    log.info("quarantine retention cron: %d records deleted (older than %d days)",
                             deleted, rd)
                await db.settings.update_one(
                    {"_key": "quarantine_retention_last_run"},
                    {"$set": {"_key": "quarantine_retention_last_run", "date": today,
                              "deleted": deleted, "retention_days": rd,
                              "auto_delete_enabled": enabled, "at": now.isoformat()}},
                    upsert=True,
                )
        except Exception as ex:
            log.warning("quarantine retention cron error: %s", ex)
        await asyncio.sleep(3600)
...
# v44.00.33 — Karantina Ayarları (retention, thresholds, spam-learn davranışı)
_QUARANTINE_SETTINGS_KEY = "quarantine_settings"
_QUARANTINE_SETTINGS_DEFAULT = {
    "retention_days": 30,               # kaç günden eski karantina otomatik silinsin
    "auto_delete_enabled": True,        # retention cron'u aktif mi?
    "spam_score_threshold": 5.0,        # bu skor ve üzeri "spam" verdict
    "high_spam_score_threshold": 10.0,  # bu skor ve üzeri "high_spam" verdict
    "auto_train_bayes_on_release": True,   # "Spam Değil" → Bayes ham eğit
    "auto_whitelist_on_release": True,     # "Spam Değil" → göndericiyi Beyaz Liste'ye ekle
    "auto_train_bayes_on_report": True,    # "Spam Öğret" → Bayes spam eğit
    "auto_blacklist_on_report": True,      # "Spam Öğret" → göndericiyi Kara Liste'ye ekle
}


@api.get("/quarantine/settings")
async def quarantine_settings_get(request: Request, license_key: Optional[str] = None):
    """Karantina ayarlarını getirir (retention/eşikler/spam-öğret davranışı).
    Boşsa default değerler döner."""
    doc = await db.settings.find_one({"_key": _QUARANTINE_SETTINGS_KEY}, {"_id": 0}) or {}
    merged = {**_QUARANTINE_SETTINGS_DEFAULT, **{k: v for k, v in doc.items() if k not in ("_key",)}}
    return merged


class QuarantineSettingsIn(BaseModel):
    retention_days: Optional[int] = None
    auto_delete_enabled: Optional[bool] = None
    spam_score_threshold: Optional[float] = None
    high_spam_score_threshold: Optional[float] = None
    auto_train_bayes_on_release: Optional[bool] = None
    auto_whitelist_on_release: Optional[bool] = None
    auto_train_bayes_on_report: Optional[bool] = None
    auto_blacklist_on_report: Optional[bool] = None


@api.post("/quarantine/settings")
async def quarantine_settings_post(payload: QuarantineSettingsIn, request: Request,
                                    license_key: Optional[str] = None):
    """Karantina ayarlarını kaydet. Master-only.
    retention_days=0 → auto-delete kapatılır (auto_delete_enabled=false zorlanır)."""
    await _require_master(request, license_key)
    updates = {k: v for k, v in payload.model_dump().items() if v is not None}
    # Validation
    if "retention_days" in updates:
        rd = int(updates["retention_days"])
        if rd < 0 or rd > 3650:
            raise HTTPException(400, "retention_days 0-3650 arası olmalı")
        updates["retention_days"] = rd
    for k in ("spam_score_threshold", "high_spam_score_threshold"):
        if k in updates:
            v = float(updates[k])
            if v < 0 or v > 100:
                raise HTTPException(400, f"{k} 0-100 arası olmalı")
            updates[k] = v
    updates["_key"] = _QUARANTINE_SETTINGS_KEY
    updates["updated_at"] = _iso()
    await db.settings.update_one(
        {"_key": _QUARANTINE_SETTINGS_KEY},
        {"$set": updates}, upsert=True,
    )
    doc = await db.settings.find_one({"_key": _QUARANTINE_SETTINGS_KEY}, {"_id": 0}) or {}
    return {**_QUARANTINE_SETTINGS_DEFAULT, **{k: v for k, v in doc.items() if k not in ("_key",)}}


@api.post("/quarantine/settings/apply-retention")
async def quarantine_apply_retention(request: Request, license_key: Optional[str] = None):
    """Retention'ı ŞİMDİ uygula: retention_days'ten eski karantina kayıtlarını sil.
    Master-only. Cron zaten günlük çalıştırıyor, bu manuel tetikleme."""
    await _require_master(request, license_key)
    doc = await db.settings.find_one({"_key": _QUARANTINE_SETTINGS_KEY}, {"_id": 0}) or {}
    settings = {**_QUARANTINE_SETTINGS_DEFAULT, **{k: v for k, v in doc.items() if k not in ("_key",)}}
    rd = int(settings.get("retention_days") or 30)
    if rd <= 0:
        return {"ok": True, "deleted": 0, "note": "retention_days=0, silme yapılmadı"}
    cutoff = (datetime.now(timezone.utc) - timedelta(days=rd)).isoformat()
    r = await db.quarantine.delete_many({"received_at": {"$lt": cutoff}})
    return {"ok": True, "deleted": r.deleted_count, "cutoff": cutoff, "retention_days": rd}
...
async def engines_cascade_preview(name: str, request: Request, license_key: Optional[str] = None):
    """v44.00.10 — Master motor toggle'ı yapmadan ÖNCE kaç bayiye yansıyacağını
...
async def engines_toggle(name: str, request: Request, license_key: Optional[str] = None):
    ...
    # v44.00.10 — MASTER CASCADE: Master motor toggle'ı YAPARSA, aynı motor
...
async def _smtp_settings(owner_license_key: Optional[str] = None) -> dict:
    """v44.00.10 — Per-tenant SMTP settings.
...
async def get_smtp_settings(request: Request, license_key: Optional[str] = None):
    # v44.00.10 — Per-tenant SMTP: bayi kendi ayarını görür (yoksa master fallback).
    scope = await _tenant_scope(request, license_key)
    owner = scope["owner_license_key"] or ""
    # Bayi kendi dokümanı
    doc = None
    ...
    return {
        ...
        "from_addr": doc.get("from_addr", ""),
        "use_tls":   doc.get("use_tls", "starttls"),
        # v44.00.10 — UI için: bayi master'ınkinden mi devraldı?
...
async def put_smtp_settings(payload: SmtpSettingsIn, request: Request, license_key: Optional[str] = None):
    # v44.00.10 — Kayıt, çağıranın tenant scope'una gider (bayi = kendi, master = global default).
    scope = await _tenant_scope(request, license_key)
    owner = scope["owner_license_key"] or ""
...
async def mail_send_test(payload: MailTestIn, request: Request, license_key: Optional[str] = None):
    ...
    v44.00.10 — Tenant-scoped: bayi kendi SMTP ayarlarıyla test eder."""
...
def _read_panel_version() -> str:
    ...
    """
    _PACKAGE_VERSION = "v44.00.52"  # backend bundle içindeki varsayılan (VERSION dosyası bulunamazsa)
...
async def version_current():
    ...
    return doc or {"version": "44.00.05", "installed_at": _iso()}
...
async def version_check_update():
    ...
    cur = await db.settings.find_one({"_key": "version"}, {"_id": 0, "_key": 0}) or {"version": "44.00.05"}
...
async def plugin_signal_log(request: Request, limit: int = 20, license_key: Optional[str] = None):
    """Master panelde son N plugin sinyalinin listesi + handled durumu.
    v44.00.10 — SECURITY: master-only. Bu liste bayi lisans key'lerini içerir,
    bayilere ASLA sızmamalı. Ayrıca kayıtlar `license_key` alanına göre filtreliyor
    değildi → bayı çağırınca master'ın kayıtları da görünüyordu. Şimdi 403.
    """
    m = await _is_master(request, license_key)
    if not m.get("is_master"):
        raise HTTPException(403, "Master yetkisi gerekli")
    items = []
...
async def version_publish(payload: VersionPublishIn, request: Request):
    ...
    if not version:
        ...
        if master_lic and master_lic.get("last_heartbeat_version"):
            ...
            version = (installed.get("version") or "44.00.05").lstrip("v")
...
async def licenses_list(request: Request, license_key: Optional[str] = None):
    # v44.00.10 — SECURITY: master-only. Bayilere lisans listesini SIZDIRMA.
    m = await _is_master(request, license_key)
    if not m.get("is_master"):
...
class HeartbeatPayload(BaseModel):
    ...
    ip: str
    hostname: Optional[str] = ""
    # v44.00.10 — Default'ı boş yap; eğer plugin göndermediyse UI 'v?' göstersin,
    ...
    class Config:
        populate_by_name = True  # hem 'version' hem 'plugin_version' kabul et
...
# v44.00.10 — Alias endpoint: `gws-simple-push` timer /api/plugin/heartbeat
...
async def license_violations(request: Request, limit: int = 100, license_key: Optional[str] = None):
    # v44.00.10 — SECURITY: master-only. İhlaller license_key + IP bilgisi içerir → sızmasın.
    m = await _is_master(request, license_key)
    if not m.get("is_master"):
        raise HTTPException(status_code=403, detail="Master yetkisi gerekli")
...
async def license_simulate_violation(p: SimulateViolation):
    v = LicenseViolation(
        ...
        reason=p.reason, version="44.00.05",
        raw={"simulated": True},
    ).model_dump()
    await db.violations.insert_one(v)
    await db.logs.insert_one(ActivityLog(
...
class DelistRequest(BaseModel):
    ...
    notes: Optional[str] = ""
    # v44.00.10 — Tenant isolation: hangi lisansa/bayiye ait olduğu (master için MASTER_LICENSE_KEY)
...
async def blacklist_delist(payload: DelistRequestIn, request: Request, license_key: Optional[str] = None):
    ...
    # v44.00.10 — Tenant sahibini belirle (master ise MASTER_LICENSE_KEY, bayi ise kendi key'i)
    ...
    for code in payload.provider_codes:
        ...
        req = DelistRequest(
            ...
            owner_license_key=owner_lk,  # v44.00.10 — Tenant isolation
        ).model_dump()
...
async def blacklist_requests(request: Request, license_key: Optional[str] = None):
    # v44.00.10 — Tenant isolation: bayi sadece KENDİ delisting taleplerini görür
    scope = await _tenant_scope(request, license_key)
    q: dict = {}
    if not scope.get("is_master"):
        owner_lk = scope.get("owner_license_key") or ""
...
async def _authorize_delist(req_id: str, request: Request, license_key: Optional[str]):
    """v44.00.10 — Kayıt sahipliğini doğrula (master her şeye erişebilir)."""
    scope = await _tenant_scope(request, license_key)
...
PLAN_FEATURES_DEFAULT = {
    "starter": {
        ...
        "installation_guide": True,  # v44.00.10 — Kurulum Rehberi (herkes görebilir)
        "install_guide": True,       # v44.00.10 — App.js aynı feature key kullanıyor (alias)
        "module_tour": True,         # v44.00.10 — Modül Turu (herkes görebilir)
        "promo_video": True,         # v44.00.10 — Tanıtım videosu (herkes görebilir)
        # === Liste Yönetimi ===
        ...
        "threat_defense_center": False,  # v44.00.10 — Threat Defense Hub (28 modül)
        "threat_defense": False,     # v44.00.10 — App.js alias (starter'da kapalı — pro/enterprise açık)
        ...
        "bounce_digest_export": False,  # v44.00.10 — CSV/XLSX export
        # === Bildirim & Raporlama ===
        ...
        "push_health_widget": True,  # v44.00.10 — Exim push health (herkes görebilir)
        "reseller_analytics_widget": True,  # v44.00.10 — Kişisel Koruma Panosu (herkes)
        # === Güvenlik & Kilit ===
        "idle_lock": True,           # v44.00.10 — Otomatik kilit + PIN
        "pin_change_request": True,  # v44.00.10 — Bayi PIN değişikliği talebi
        # === Yönetim ===
    ...
    "pro": {
        ...
        "installation_guide": True, "install_guide": True,
        "module_tour": True, "promo_video": True,
        # Liste
        ...
        "threat_intel": True, "threat_defense_center": True, "threat_defense": True,
    ...
    "enterprise": {
        # v44.00.10 — ENTERPRISE'da TÜM özellikler AÇIK olmalı (kullanıcı bildirdi:
...
class VerifyLicenseIn(BaseModel):
    ...
    # v44.00.10 — Plugin version (verify sırasında master hangi versiyonun bağlı olduğunu bilsin)
...
async def plugin_verify_license(payload: VerifyLicenseIn, request: Request = None):
    ...
    if payload.license_key:
        ...
        # v44.00.10 — license_key SAĞLANMIŞ ama DB'de aktif eşleşme YOK →
    ...
    if not lic:
        # v44.00.10 — browser_ip'yi de yakala (server_ip = payload.ip, browser_ip = request headers)
        _browser_ip = ""
        try:
            if request is not None:
                _browser_ip = (request.headers.get("x-forwarded-for") or "").split(",")[0].strip()
        ...
        # v44.00.10 — Canonical collection'a da yaz (frontend'in yeni list endpoint'i buradan okur)
        await db.license_violations.insert_one(dict(v))
        await db.violations.insert_one(v)
        asyncio.create_task(_fire_license_alert(v))
...
app.include_router(_notifications_router, prefix="/api")
...
app.include_router(_install_screenshots_router, prefix="/api")
