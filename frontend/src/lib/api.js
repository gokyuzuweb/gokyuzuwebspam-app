...
export const api = {
  ...
  eventsScoreTrend: (eventId, hours = 24) => client.get(`/events/score-trend/${eventId}`, { params: { hours } }).then(r => r.data),
  savedFilters: (params = {}) => client.get("/events/saved-filters", { params }).then(r => r.data),
  savedFilterCreate: (payload, params = {}) => client.post("/events/saved-filters", payload, { params }).then(r => r.data),
  savedFilterDelete: (sid, params = {}) => client.post(`/events/saved-filters/${sid}/delete`, null, { params }).then(r => r.data),
  ...
  ruleDel: (id) => client.post(`/rules/${id}/delete`).then(r => r.data),
  ruleUpdate: (id, payload) => client.post(`/rules/${id}/update`, payload).then(r => r.data),
  ...
  trustedIpsExportCsv: async () => {
    const res = await client.get("/settings/trusted-ips/export.csv", { responseType: "blob" });
    return res;
  },
  ...
  adminPlanMatrix: (licenseKey) =>
    client.get("/admin/plan-matrix",
      { params: licenseKey ? { license_key: licenseKey } : {}, withCredentials: true }).then(r => r.data),
  adminPlanMatrixSave: (matrix, licenseKey) =>
    client.post("/admin/plan-matrix", { matrix },
      { params: licenseKey ? { license_key: licenseKey } : {}, withCredentials: true }).then(r => r.data),
  ...
  idleLockGet: () => client.get("/settings/idle-lock").then(r => r.data),
  idleLockSet: (payload) => client.post("/settings/idle-lock", payload).then(r => r.data),
