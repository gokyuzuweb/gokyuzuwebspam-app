...
import { CreditCard, Check, Star, Loader2, CheckCircle2, XCircle, Copy, ArrowRight } from "lucide-react";
...
export function CheckoutSuccess() {
  ...
  const copyKey = async () => {
    if (!tx?.license_key) return;
    await navigator.clipboard.writeText(tx.license_key);
          ...
          {paid ? (
            <>
              ...
              <p className="text-slate-400 mb-6">
                Lisans anahtarınız oluşturuldu ve <b>{tx.customer_email}</b> adresine gönderildi.
              </p>
              <div className="p-4 rounded border border-indigo-500/30 bg-indigo-500/5 mb-6">
                <div className="text-[10px] uppercase tracking-widest text-slate-500 mb-1">Lisans anahtarı</div>
                <div className="mono text-indigo-300 text-sm break-all">{tx.license_key}</div>
              ...
              <div className="text-xs text-slate-500">
                Plugin'inizin WHM panelinden 'Lisansı Sorgula' butonuna basın ya da bu anahtarı elle girin.
              </div>
            </>
          ) : tx?.status === "failed" ? (
            <>
