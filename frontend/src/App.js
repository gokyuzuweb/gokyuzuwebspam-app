...
// v44.00.07 — DemoBanner artık kullanılmıyor; demo uyarısı LicenseGate içinde tek satır olarak gösteriliyor
    ...
    <aside data-testid="sidebar" className={`w-60 shrink-0 border-r border-slate-800/80 bg-gradient-to-b from-slate-950 via-slate-950 to-slate-900/60 backdrop-blur flex flex-col ${effective === "ar" ? "rtl" : ""}`}>
      <div className="h-14 flex items-center gap-2 px-3 border-b border-slate-800/80">
        ...
        <div className="leading-tight min-w-0">
          ...
          <div className="text-[10px] uppercase tracking-widest text-slate-500 mono">v44.00.07 · {effective.toUpperCase()}</div>
        </div>
      </div>
  ...
  return (
    <div className={rootCls} data-testid={inFrame ? "shell-embedded" : "shell-standalone"}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <PluginStatusStripe />
        {/* v44.00.07 — DemoBanner kaldırıldı: uyarı LicenseGate demo bandında tek satır olarak gösteriliyor */}
        <ImpersonationBar />
        <PushToastBridge />
        <BayiEventBridge />
        <RenewalBanner />
